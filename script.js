...
async function loadProjects() {
  try {
    ...
    if (target) {
      target.innerHTML =
        '<p class="load-error">Projects could not be loaded.</p>';
    }
  }
}
...
function flattenProjects() {
  return allProjectTypes.flatMap(group =>
...
function renderFeaturedProjects() {
  ...
  const favorites = flattenProjects()
    ...
    .sort((a, b) => {
      const aFavoriteOrder =
        a["favorites-order"] !== undefined
          ? Number(a["favorites-order"])
          : Number(a.order ?? Number.MAX_SAFE_INTEGER);

      const bFavoriteOrder =
        b["favorites-order"] !== undefined
          ? Number(b["favorites-order"])
  ...
  if (favorites.length === 0) {
    container.innerHTML =
      '<p class="load-error">No featured projects found.</p>';
  }
}
...
function matchesActiveFilter(project) {
  ...
  return (
    ...
    values.includes(activeFilter.value)
  );
}
...
function createProjectCard(
  ...
  const card = document.createElement(
    project.page ? "a" : "article"
  );

  card.className =
    `project-card ${extraClass}`.trim();
